...
def kim_model_so_glob(kim_model):
    return str(
        Path.home()
        / ".kim-api"
        / "*"
...
def relax_one(cif_in, cif_out, meta_path, run_dir, relax_cfg, nproc, launcher):
    ...
    kim_model = relax_cfg["kim_model"]
    hits = glob.glob(kim_model_so_glob(kim_model))
    if not hits:
        raise RuntimeError(
            f"KIM model .so not found. Try:\n"
